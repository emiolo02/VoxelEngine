...
void
Movement::Orbit(float dt) {
    static float time = 0.0f;
    time += dt;

    m_Position = m_OrbitCenter + vec3(std::sin(time * m_OrbitSpeed) * m_OrbitRadius, m_OrbitHeightOffset,
                                      std::cos(time * m_OrbitSpeed) * m_OrbitRadius);

    m_Forward = normalize(m_OrbitCenter - m_Position);
}
